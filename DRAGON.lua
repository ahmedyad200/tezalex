...
sudo_users = {SUDO,1702918704,1797771210,1720330493,944353237}   
...
sendAudio(msg.chat_id_,msg.id_,'./'..ffrr,"🎼 𝗘𝗫")  
...
if text and text:match("^رفع اكس @(.*)$") and DevSoFi(msg) then
...
if text and text:match("^رفع اكس (%d+)$") and DevSoFi(msg) then
...
if text and text:match("^تنزيل اكس @(.*)$") and DevSoFi(msg) then
...
if text and text:match("^تنزيل اكس (%d+)$") and DevSoFi(msg) then
...
if text == 'مين نصبلك' or text == 'بوت عظيم' or text == 'عايز بوت' then
local Text = [[
...
{{text = '𓆩 𝒃𝒐𝒕 𝒂𝒍𝒆𝒙 𓆪 ',url="t.me/EX_CHAN"}}, 
}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..token..'/sendMessage?chat_id=' .. msg.chat_id_ .. '&text=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end

if text == بوت اليكس' or text == 'Banda' or text == 'banda' then
local Text = [[
اقوي بوت حمايه 
]]
keyboard = {} 
...
send(msg.chat_id_, msg.id_," ❏ عذرا الملف لايدعم سورس اليكس ر\n") 
end
return false
end
...
if text and text:match("^رفع قلب اليكس(%d+)$") and DevSoFi(msg) then
local userid = text:match("^رفع قلب اليكس (%d+)$")
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
st = https.request('https://black-source.tk/BlackTeAM/ImageInfo.php?token='..tonumber(000000000)..'&url='..sticker_id.."&type=sticker")
eker = JSON.decode(st)
...
Srrt = https.request('https://black-source.tk/BlackTeAM/ImageInfo.php?token='..tonumber(000000000)..'&url='..photo_id.."&type=photo")
...
-------------------------------
if text == ""..(database:get(bot_id..'Name:Bot') or 'اكس').." غادر" or text == 'غادر' then  
...
send(msg.chat_id_, msg.id_, t..'≪━━━━━━𝗘𝗫━━━━━━≫ٴ\n❏ [ 𝗘𝗫 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 ](t.me/s_an_for1)')
end
